...
`hop <expr>` is shorthand for `hop to <expr>`, which opens the matching worktree. With the [VS Code extension](vscode/README.md) installed, the most recently focused VS Code window switches to it in place; otherwise Hop runs `code <folder>`. An expression is a pull request (`owner/repo#123`, `repo#123`, `#123`, `123`, or a PR URL) or a query. Each word of a query must match a branch, repository, folder name, or PR title, so `hop flipbook m` narrows `hop flipbook`. The word `default` matches a checkout of the repository's default branch, as reported by `origin/HEAD` or else `main` or `master`, so `hop flipbook default` opens it without naming the branch. Ambiguous results use a numbered terminal picker that lists the default-branch checkout of any repository named in the query first, then the most recently used worktrees. A PR without a local worktree is reported with its URL.
